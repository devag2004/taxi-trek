...
int wt[]={1,2,5,10,20,50,100,200,500};
int numb=10;
void findmin(int v)
{
...
void showrent()
    ...
    cout<<"\nYou can pay the fee in the following currency denominations : [1,2,5,10,20,50,100,200,500]";
...
void Welcome_window()
{
    ...
    cout << "\t\t\tWelcome to TAXI TREK\n\n\n";

    cout << "\t\t\tThe Agency is managed by: \n\n\n";

    cout << "\t\t\tDEV, VAIBHAV, PRACHI, MAYANK\n\n";
}
class Pair
{
public:
...
int main()
{
    ...
    while (endp == 1)
    {
        ...
        if (choice == 1)
        {
            ...
            char ch;
        while (true) {
        ch = _getch(); // Get a single character without echoing to the console
        if (ch == 13) // Check if Enter key is pressed
            break;
        if (ch == '\b' && !psd.empty()) { // Check if backspace key is pressed and password is not empty
            cout << "\b \b"; // Move cursor back, overwrite with space, move cursor back again
            psd.pop_back(); // Remove last entered character
            continue;
        }
        cout << "*"; // Display asterisk instead of the actual character
        psd.push_back(ch); // Append the character to the password string
    }
